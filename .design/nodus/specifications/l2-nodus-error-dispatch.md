...
there is nothing to dispatch. The uncaught error still ends the step sequence, exactly as the
dispatch branch does, and the run records `NODUS:UNHANDLED_ERROR` — an error entry naming the
failed step and its cause, with a `StepError` trace event — beside the triggering error, which
stays in `RunResult.errors`. `Status` stays `Partial`, as it is after a dispatched handler.
`W001`'s validator warning ("No @err handler...") still fires only for the true-absence case, and
now states what the run will do.

This is what `l1-nodus-language.md` §4.5 requires: an unhandled error terminates the workflow with
`NODUS:UNHANDLED_ERROR`. The shipped loop once ran the remaining steps against whatever the
failed step left behind, so omitting `@err:` made a failure *less* contained than declaring
one — the inverse of what the language promises, and the outcome §6 rejects for the handler
case. Ending the sequence on both paths removes the inversion; the compensation unwind arms on
both (`l2-nodus-compensation.md`, NL-22(d)).
...
| --- | --- | --- | --- |
| 1.1.0 | 2026-09-24 | Core Team | Realization sync (2026-09-24): §4.5: with no `@err:` handler declared, an uncaught error now ends the step sequence and is recorded as `NODUS:UNHANDLED_ERROR`, as the language requires; the deviation recorded as pending is closed. |
| 1.0.2 | 2026-09-24 | Core Team | Consistency pass (2026-09-24): §4.5 records the deviation from L1 §4.5: with no `@err:` handler declared the run keeps executing after an uncaught error instead of terminating with `NODUS:UNHANDLED_ERROR`, so omitting the handler makes a failure less contained than declaring one. The NL-9 row drops a test count. |
