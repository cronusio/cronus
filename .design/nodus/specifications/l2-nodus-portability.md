...
- [l2-nodus-commands.md](l2-nodus-commands.md) — [ADDED v1.13.0] specifies the generic host-tool extension point §4.9.1 names as the prerequisite for `ToolUse`: one `CommandProvider` role (the tenth `ExtensionRole`), `EffectClass::ToolUse` realized fail-safe, and the LP-3 admission record for the new seam (§4.8.3 points to it)
...
| LP-5 Composable extension | **Implemented, with two single-seam entry points left.** The seams are independent units and they compose: `run_with_options` takes the model, audit, dialog, policy, settlement, host vocabulary, capability manifest, run metadata, execution mode and exposure switches in one call, with the built-ins as defaults, so a real model runs under a host policy and a real rail under one too (`l2-nodus-runtime.md` §4.5). There is no global mutable state and no inheritance; combinators are pure functions in `workflows.rs`. The single-seam entry points (`run_with_provider`, `run_with_audit`, `run_with_dialog`, …) remain as conveniences over the same executor. Still single-seam: `run_with_environment*` and `run_with_config*`, which take their own seam and fix the model to `StubProvider`; folding them into `RunOptions` is what remains. |
...
| Settlement | `SettlementRail` | `NoopSettlementRail` | Implemented (`portability.rs`; `l2-nodus-settlement.md`) [row ADDED v1.13.0 — it had never been listed here] | **Yes** — `handle_settlement` consults it after the LP-11 gate |
| Command | `CommandProvider` | `SimulatedCommands` | **Specified** (`commands.rs`; `l2-nodus-commands.md`) [ADDED v1.13.0] | **No** — not yet built; once wired it answers every command no other role owns |
...
#### 4.8.3 `CommandProvider` (LP-11 `tool_use`) — **LP-3 SATISFIED** [ADDED v1.13.0]

The record lives in the realizing spec, as L1 §4.14 requires: `l2-nodus-commands.md` §4.11. Its two contexts are the operator's own machine and a confined sandbox — two execution worlds whose decision shapes (friction-graduated asking versus deny-by-default classified refusal) and fail directions differ, and which the host's own `l1-execution-locus.md` (LOC-4, LOC-7) already requires a single command consumer to span. The interface names no host type. Disposition: **satisfied**; the seam may be wired once `l2-nodus-commands.md` is built.
...
**[UPDATED v1.13.0]** The prerequisite this paragraph names is now specified: `l2-nodus-commands.md` introduces the generic host-tool extension point (`CommandProvider`, the tenth role) and realizes `ToolUse` fail-safe (its §4.2) — every command that is neither core-owned nor owned by another role is gated as `tool_use`, host-declared commands included, which supersedes the exemption in the last sentence above. Specified, not built: until it is, this section describes the crate as it stands.
...
define — **[UPDATED v1.13.0]** `l2-nodus-commands.md` defines it (pending build). `gate` values beyond the two effect-class strings — a workflow declaring its own
...
| --- | --- | --- | --- |
| 1.13.0 | 2026-09-24 | Core Team | Design pass (2026-09-24): `l2-nodus-commands.md` specifies the generic host-tool extension point §4.9.1 named as the prerequisite for `ToolUse` — one `CommandProvider` role (the tenth `ExtensionRole`) and `EffectClass::ToolUse` realized fail-safe. §4.9.1, §4.9.6 and the LP-11 row record that it is specified; §4.2's registry gains the `Command` row and the `Settlement` row it had never carried; §3's LP-3 row and a new §4.8.3 record `CommandProvider`'s disposition — **satisfied**, on the record kept in the realizing spec, as L1 §4.14 requires. Nothing is built: every other row still describes the crate as it is. |
| 1.12.0 | 2026-09-24 | Core Team | Realization sync (2026-09-24): LP-5 implemented for every seam the composed entry point takes (`run_with_options`); the environment and config entry points remain single-seam. LP-11: a policy gate now governs a real model call and a real settlement rail through the composed entry point. |
| 1.11.2 | 2026-09-24 | Core Team | Consistency pass (2026-09-24): LP-5 → Partial: every seam added after the model/audit pair arrived as its own entry point fixing the rest to built-ins, so no two compose and none composes with a real model. LP-11 notes the gate never meets a real effect and that `@err:` dispatch now exists (the row still called it unbuilt). LP-17 points to the settlement composition gap. |
