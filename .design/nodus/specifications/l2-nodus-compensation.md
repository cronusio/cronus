...
- [l2-nodus-commands.md](l2-nodus-commands.md) — [ADDED v1.1.1] the command-execution seam a compensation now reaches: gated as `tool_use`, keyed in its own namespace, never memoized (§4.6, §4.7 there)
...
- **The LP-11 `decide → effect → observe` gate composition.** NL-22 requires each compensation to pass that gate "like any effect". When this spec was written no such gate existed; it has since landed (`l2-nodus-portability.md`, LP-11 row), and compensations inherit it as predicted, because they route through the one `execute_command` path every effect takes — no second mechanism was retrofitted. What they inherit includes that gate's composition limit: a run with a real policy runs on the stub model (`l2-nodus-runtime.md` §4.5).
...
| NL-22(b) Each compensation is a fallible gated effect | A compensation runs through the same `execute_command` path as any command, so it is subject to the same rule checks and emits the same events. A compensation that errors pushes `NODUS:COMPENSATION_FAILED` onto `ctx.errors`, leaving the original effect **live** — never swallowed, never reported as undone. **[UPDATED v1.1.1]** Under the command-execution seam (`l2-nodus-commands.md`) a compensation is answered like any seam-owned command: gated as `tool_use`, executed by the host's `CommandProvider` with an effect key in the `comp/<step>` namespace, and never memoized, since it runs outside any retry scope (§4.6, §4.7 there). Specified, not built. |
...
| 1.1.1 | 2026-09-24 | Core Team | Design pass (2026-09-24): NL-22(b)'s row records that, under the command-execution seam specified in `l2-nodus-commands.md`, a compensation reaches the host's `CommandProvider` like any seam-owned command — gated as `tool_use`, keyed in its own namespace, never memoized — so a real undo can run. Nothing changes in the crate; nothing is built. |
